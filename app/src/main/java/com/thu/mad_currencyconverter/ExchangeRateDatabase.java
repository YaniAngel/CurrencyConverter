...
public class ExchangeRateDatabase {
    ...
    private final static double[] EXCHANGE_LIST;

    static {
        EXCHANGE_LIST = new double[CURRENCIES_LIST.length];
        for(int i=0; i< CURRENCIES_LIST.length; i++)
        {
            EXCHANGE_LIST[i] = CURRENCIES_MAP.get(CURRENCIES_LIST[i]).getRateForOneEuro();
        }
    }
    ...
    /*public double[] getExchangeRate()
    {
        ...
        for(int i=0; i<CURRENCIES_LIST.length; i++)
        {
            result[i] = RATES[i].getRateForOneEuro();
        }
        return result;
    }*/
    /**
